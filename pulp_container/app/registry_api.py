...
class Blobs(RedirectsMixin, ContainerRegistryApiMixin, ViewSet):
    ...
    def handle_safe_method(self, request, path, pk):
        ...
        if not blob:
            ...
            if not blob:
                # Try to fetch the blob from the upstream if pull-through is available
                if (
                    distribution.remote
                    ...
                    # This raises a BlobNotFound if the blob is not found on the upstream
    ...
    def fetch_blob(self, remote, pk):
        """Check if the blob exists on the upstream remote."""
...
class Manifests(RedirectsMixin, ContainerRegistryApiMixin, ViewSet):
    ...
    def handle_safe_method(self, request, path, pk):
        ...
        if pk[:7] != "sha256:":
            # fetching manifest by tag name
            tag = models.Tag.objects.filter(name=pk, pk__in=repository_version.content).first()
            # If pull-through is available, must check to see if there have been updates to the tag
            if (
                distribution.remote
                and distribution.pull_through_distribution
                and permission_checker.has_pull_through_permissions(distribution)
                ...
                try:
                    ...
                    # if we fail to fetch the manifest on the upstream and we don't have a local tag
                    # already saved, then continue raising the HTTP exception
                    if not tag:
                        raise e
                else:
                    # The tag exists on the upstream remote, but the manifest it points to does not
                    # exist in Pulp. Create a fake manifest and tag object for our response and
                    # dispatch a task in the background to download the manifest, its blobs and tag
                    # and save them to Pulp.
                    if local_manifest is None:
                        ...
                        # The tag exists on the upstream remote and the manifest it points to exists in Pulp.
                        # If the tag is not already saved in Pulp or if the manifest it points to has changed,
                        # then save the tag and manifest to Pulp.
            ...
            if not manifest and (
                ...
                manifest.touch()
            # Only check the upstream if we don't have it locally and there is a pull-through distro
            if (
                not manifest
                ...
                if manifest:
                    # Manifest is already stored in Pulp, make sure it is added to the repository
                    ...
                    # Manifest is not stored in Pulp, create a fake manifest and dispatch a task
                    # in the background to download the manifest, its blobs and save them to Pulp.
    ...
    def fake_init_manifest(self, response, pk):
        """
        Populate a manifest with data from an upstream response.
        This manifest is not meant to be saved to Pulp, only used for a ManifestResponse when
        using pull-through caching.
        """
