...
def get_or_create_blob(layer_json, manifest, path):
    ...
    try:
        ...
        blob.save()
        ContentArtifact(artifact=layer_artifact, content=blob,
...
def add_image_from_directory_to_repository(path, repository, tag):
    ...
    manifest = Manifest(digest=manifest_digest, schema_version=2,
                        media_type=MEDIA_TYPE.MANIFEST_OCI)
    manifest.save()
    ContentArtifact(artifact=manifest_artifact, content=manifest,
...
def build_image_from_containerfile(containerfile_pk=None, artifacts={}, repository_pk=None,
                                   tag=None):
    """
