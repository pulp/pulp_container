...
class ContainerFirstStage(Stage):
    ...
    async def _check_for_existing_manifest(self, head_manifest_task):
        """
        See if upstream manifest exists in Pulp. If not download the manifest and return the data.

        This function expects a task from a HEAD request to the upstream manifest. _process_manifests
        creates these tasks and waits for them in parallel. We do HEAD checks first to avoid hitting
        the upstream's pull limits for images we already have.
        """
        response = await head_manifest_task

        digest = response.headers.get("docker-content-digest")
        # The url is http://<upstream_url>/v2/<repo_name>/manifests/<reference>
        url = response.url
                ...
                if media_type in (MEDIA_TYPE.MANIFEST_LIST, MEDIA_TYPE.INDEX_OCI):
                    ...
                    for listed_manifest_task in asyncio.as_completed(
                        ...
                        for listed_manifest in list_dc.extra_data["listed_manifests"]:
                            listed_manifest_digest = listed_manifest["manifest_dc"].content.digest
                            self._synced_digests[listed_manifest_digest].append(manifest_ref)
