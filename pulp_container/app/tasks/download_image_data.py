...
def download_image_data(repository_pk, remote_pk, text_manifest_data, tag_name):
    ...
    first_stage = ContainerPullThroughFirstStage(
        remote, text_manifest_data.encode("utf-8"), tag_name
    )
...
class ContainerPullThroughFirstStage(ContainerFirstStage):
    ...
    async def run(self):
        ...
        content_data = json.loads(self.raw_manifest_data)
        ...
        if media_type in (MEDIA_TYPE.MANIFEST_LIST, MEDIA_TYPE.INDEX_OCI):
            list_dc = self.create_tagged_manifest_list(
                self.tag_name, content_data, self.raw_manifest_data, media_type
            )
