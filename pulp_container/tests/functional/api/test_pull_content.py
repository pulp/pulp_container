...
class TestPullContent:
    ...
    def setup(
        ...
        * `Pulp #4460 <https://pulp.plan.io/issues/4460>`_
        ...
        type(self).distribution_with_repo = container_distribution_factory(
            repository=self.repo.pulp_href
        )
        type(self).distribution_with_repo_version = container_distribution_factory(
            repository_version=self.repo.latest_version_href
        )

    def test_api_returns_same_checksum(self, container_bindings):
        ...
        blob_checksums = [
            c.digest
            for c in container_bindings.ContentBlobsApi.list(
                repository_version=self.repo.latest_version_href
            ).results
        ]
    ...
    def test_api_performes_schema_conversion(self, bindings_cfg):
    ...
    def test_create_empty_blob_on_the_fly(self, bindings_cfg):
        """
    ...
    def test_pull_image_from_repository(self, local_registry, registry_client):
        ...
        """
        local_registry.pull(self.distribution_with_repo.base_path)
        local_image = local_registry.inspect(self.distribution_with_repo.base_path)
    ...
    def test_pull_image_from_repository_version(self, local_registry, registry_client):
        """Verify that a client can pull the image from Pulp.
        ...
        4. Ensure image is deleted after the test.
        """
        local_registry.pull(self.distribution_with_repo_version.base_path)
        local_image = local_registry.inspect(self.distribution_with_repo_version.base_path)
    ...
    def test_pull_image_with_tag(self, local_registry, registry_client):
        ...
        4. Ensure image is deleted after the test.
        """
        local_registry.pull(self.distribution_with_repo.base_path + PULP_HELLO_WORLD_LINUX_TAG)
        local_image = local_registry.inspect(
            self.distribution_with_repo.base_path + PULP_HELLO_WORLD_LINUX_TAG
        )
    ...
    def test_pull_nonexistent_blob(self, bindings_cfg):
        """
...
class TestPullOnDemandContent:
    ...
    def setUpClass(
        ...
        * `Pulp #4460 <https://pulp.plan.io/issues/4460>`_
        ...
        type(self).artifact_count = pulpcore_bindings.ArtifactsApi.list().count
    ...
    def test_pull_image_from_repository(self, local_registry, registry_client, pulpcore_bindings):
        ...
        assert new_artifact_count > self.artifact_count
