...
def test_blob_upload_status(container_bindings, full_path, add_to_cleanup):
    """Test GET blob upload status returns current upload progress."""
    repo_name = "test/upload_status"
    upload_url = urljoin(
        container_bindings.client.configuration.host,
        f"/v2/{full_path(repo_name)}/blobs/uploads/",
    )
    auth = get_auth_for_url(upload_url)

    response = requests.post(upload_url, auth=auth)
