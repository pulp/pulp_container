...
from pulp_container.app.checks import pull_through_distribution_check
...
class TestPullThroughDistributionRepair(TestCase):
    ...
    def test_check_warns_until_distribution_is_repaired(self):
        ContainerPullThroughDistribution.objects.create(name="legacy", base_path="legacy")

        warnings = pull_through_distribution_check(None)

        self.assertEqual(warnings[0].id, "pulp_container.W001")
